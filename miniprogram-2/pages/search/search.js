...
Page({
  ...
  //根据输入的名称和场馆类型id筛选总场馆（如无场馆类型则输入0）
  getVenue: function (searchName,courttypeid) {
    var that = this
    console.log(searchName)
    console.log(courttypeid)
    if (searchName != null) {
      wx.request({
        url: DjangoURL + 'wx/venue/searchVenue?cityid=' + that.data.cityid + '&searchName=' + searchName + '&longitude=' + that.data.longitude + '&latitude=' + that.data.latitude+'&page='+that.data.page+'&courttypeid='+courttypeid,
        method: "GET",
        success: function (res) {
          that.setData({
            venue: that.data.venue.concat(res.data.data),
